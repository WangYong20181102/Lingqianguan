...
import com.wtjr.lqg.lock.Constants;
...
//		screenDispaly = AppUtil.getScreenDispaly(context);
        ...
        screenDispaly = app.mScreenWidth;
        paint = new Paint(Paint.DITHER_FLAG);// 创建一个画笔
        bitmap = Bitmap.createBitmap(screenDispaly, screenDispaly, Bitmap.Config.ARGB_8888); // 设置位图的宽高
        canvas = new Canvas();
        canvas.setBitmap(bitmap);
        paint.setStyle(Style.STROKE);// 设置非填充
//        paint.setStrokeWidth(4 * app.mDensity);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (isGesturesTrackColor) {
            paint.setColor(Color.parseColor("#ffca56"));// TODO 设置默认连线颜色
        } else {
            paint.setColor(Color.TRANSPARENT);// TODO 设置默认连线颜色
        }
    ...
    private void drawErrorPathTip() {
        canvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
        if (isGesturesTrackColor) {
            paint.setColor(Color.RED);// TODO 设置默认连线颜色
        } else {
            paint.setColor(Color.TRANSPARENT);// TODO 设置默认连线颜色
        }
        for (Pair<GesturePoint, GesturePoint> pair : lineList) {
